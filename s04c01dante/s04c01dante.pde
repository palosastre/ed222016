...
// Libreta
/*
Una vez que corre el programa se dibujarán una serie de figuras (elipses, lineas y triángulos) que forman un patrón que consta de un fondo color violeta claro, 
 sobre este hay una serie de triángulos de color rosado oscuro con líneas de un color rosado claro. Estos forman figuras similares a las de un coco de nieve en el contorno de cada módulo.
 ...
 Sobre cada modulo, además se dibujará una cruz en el centro de la figura del tamaño de cada módulo con un grosor de 0.3px de color rosado.
 Si se presiona la letra "s", se guardará la imagen en formato pdf con nombre "s04c00dante(número de imagen).pdf" dentro de la carpeta frames la cual esta contenida dentro de la carpeta del archivo.
 */
import processing.pdf.*;
void setup() {          //  Función que se ejecuta al principio una sola vez

  size(900,750);         // Se crea un canvas de medidas 1200x600 px
...
void draw() {        // función que dibuja, y se repite solamente siesque se presiona la tecla r
  beginRecord(PDF, "image/s04c01Dante.pdf"); // se empieza a guardar un pdf con lo que sigue



  for (int x = 30; x < width; x += 120) { //mientras la nueva variable x sea menor que el ancho, se ejecutará el código de abajo e irá sumando 120 a x cada vez.
    for (int y = 15; y < height; y += 120) { //mientras la nueva variable y sea menor que el ancho, se ejecutará el código de abajo e irá sumando 120 a y cada vez.
       ...
       */
      triangle(x, y, x, y+30, x+10, y+15);// triangulos de la izquierda
      ...
      if (r<15 && r>25) {   //si es que la variable r es menor a 15 o es mayor a 25 se ejecutará lo siguiente
      ...
      } else if (r>5 && r<35) {   // si no se cumple la condición anterior, y r es mayor que 5 o r es menor que 35 que se ejecute lo siguiente
...
void keyPressed() {  // se crea la funcion keyPressed
  ...
  if(key == 's'){  //si es que se presiona la letra s que ocurra lo siguiente
    saveFrame("image_tiff/s04c01danteTiff###.pdf");}  //guarda el frame como tiff
}