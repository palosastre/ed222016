...
void setup() {
  ...
  size(900, 1500);  // tamaño de nuestra mesa de trabajo
  ...
  for (int x = 0; x < height; x= x+5) {
    ...
    if (x < 50) {  // cuando x es menos de 50
      strokeWeight(l); // cambia el grosor de la linea al random "l"
    }
    
    line(1, x*12, 1024, x*12); // esto corespondo el largo y posicion que tendra nuestra linea
  }
