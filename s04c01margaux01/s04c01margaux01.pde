...
void setup(){            //anunciación de los predeterminados
  background(#00BC9C);   //color del fundo
  size(300, 250);        //tamaño del canvas
} 
...
void draw() {            //formas en todo el canvas 
  beginRecord(PDF, "s04c01margaux01.pdf");      //empiezar el registro del pdf en la carpeta
  for (int y = 0; y < height; y +=20) {       //distancia de 20px en la altura
    for (int x = 0; x < width; x +=20) {      //distancia de 20px en la anchura
      ...
      noFill();                               //sin relleno
      rect(x, y, i, j);                       //rectangulo que forma una cuadrícula
 }
}
