...
int blanco;       //creo variable llamada "blanco".
...
void draw() {
  beginRecord(PDF, "s04/s04c00isi01###.pdf"); //comienza a grabar.
  ...
  noLoop();    // imagen fija.
  endRecord(); //para de grabar.
}


void keyPressed() {
  //condicional 1:  
  if ( key == 'a' ) {           //si pulso la tecla 'a'.
    redraw();                   //se vuelve a dibujar.
    //condicional 2:
  } else if ( key == 's' ) {                //si pulso la tecla 's'.
    saveFrame ( "file/s04c00isi01.pdf" );   //se guarda con este nombre.
    //condicional 3:
